...
class ExperimentRegistry:
    ...
    @classmethod
    def discover_executors(cls, package_name: str) -> "ExperimentRegistry":
        registry = cls()
        registry.load_executors(package_name)
        return registry

    def load_executors(self, package_name: str) -> None:
        ...
        for module_info in pkgutil.iter_modules(
            ...
            experiment_type = _experiment_type_from_module_name(module_name)
            executor = _create_executor_from_module(module)
    ...
    def execute(self, experiment_type: str, context: dict) -> dict:
...
def _experiment_type_from_module_name(module_name: str) -> str:
    return module_name.removesuffix("_executor").upper()


def _create_executor_from_module(module: ModuleType) -> ExperimentExecutor:
    ...
    if factory is not None:
        ...
        return _validate_executor(factory(), module.__name__)

    executor_class = getattr(module, "EXECUTOR_CLASS", None)

    if executor_class is not None:
        return _instantiate_executor(executor_class, module.__name__)
    ...
    return _instantiate_executor(executor_classes[0], module.__name__)


def _instantiate_executor(
    executor_class: Callable[[], ExperimentExecutor],
    module_name: str,
) -> ExperimentExecutor:
    return _validate_executor(executor_class(), module_name)
