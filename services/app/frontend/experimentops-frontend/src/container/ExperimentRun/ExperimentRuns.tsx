...
import { CompareExperimentRunsDialog } from './CompareExperimentRunsDialog'
  ...
  return (
    <>
      <ProjectFrame
        ...
        project={container.projectQuery.data}
      >
          ...
          {!container.canListExperimentRuns ? (
            ...
            <DataTable
              columns={getExperimentRunColumns({
                isRunSelected: container.isRunSelected,
                onToggleRun: container.handleToggleRun,
                selectable: true,
                t,
              })}
              ...
              toolbarEnd={
                container.selectedRunCount >= 2 ? (
                  <button
                    ...
                    disabled={!container.canCompareExperimentRuns}
                    onClick={() => container.setIsCompareDialogOpen(true)}
                    title={
                      container.canCompareExperimentRuns
                        ? undefined
                        : t('runs.compare.permissionDenied')
                    }
                    type="button"
                  >
                    {t('runs.compare.compareRuns', {
                      count: container.selectedRunCount,
                    })}
                  </button>
                ) : null
              }
      ...
      {container.isCompareDialogOpen ? (
        <CompareExperimentRunsDialog
          axis={container.comparisonAxis}
          isComparing={container.compareExperimentRunsMutation.isPending}
          onAxisChange={container.setComparisonAxis}
          onClose={() => container.setIsCompareDialogOpen(false)}
          onCompare={container.handleCompareRuns}
          selectedCount={container.selectedRunCount}
        />
      ) : null}
    </>
  )
